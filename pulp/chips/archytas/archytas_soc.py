...
class ArchytasSoc(gvsoc.systree.Component):
    def __init__(self, parent, name, parser, binary):
        ...
        l2_mem = memory.memory.Memory(self, 'test-mem', size=ArchytasArch.L2_SIZE)

        # Create Tiles
        cluster:List[Archytas_T_Tile] = []
        for id in range(0,ArchytasArch.NB_CLUSTERS):
            cluster.append(Archytas_T_Tile(self, f'magia-tile-{id}', parser, id))
        ...
        if (ArchytasArch.ENABLE_NOC):
            ...
            #noc.o_NARROW_MAP(cluster[0].i_NARROW_INPUT(),base=ArchytasArch.L1_ADDR_START+ArchytasArch.L1_TILE_OFFSET*0,size=ArchytasArch.L1_SIZE,x=1,y=0,rm_base=False)
            #noc.o_NARROW_MAP(cluster[1].i_NARROW_INPUT(),base=ArchytasArch.L1_ADDR_START+ArchytasArch.L1_TILE_OFFSET*1,size=ArchytasArch.L1_SIZE,x=2,y=0,rm_base=False)
            #noc.o_NARROW_MAP(cluster[2].i_NARROW_INPUT(),base=ArchytasArch.L1_ADDR_START+ArchytasArch.L1_TILE_OFFSET*2,size=ArchytasArch.L1_SIZE,x=1,y=1,rm_base=False)
            #noc.o_NARROW_MAP(cluster[3].i_NARROW_INPUT(),base=ArchytasArch.L1_ADDR_START+ArchytasArch.L1_TILE_OFFSET*3,size=ArchytasArch.L1_SIZE,x=2,y=1,rm_base=False)

            # Bind memory to noc
            #noc.o_NARROW_MAP(l2_mem.i_INPUT(),base=0,size=ArchytasArch.L2_SIZE,x=0,y=0,rm_base=True)
            noc.o_NARROW_MAP(l2_mem.i_INPUT(),name='l2map',base=0,size=ArchytasArch.L2_SIZE,x=0,y=0,rm_base=False)
            ...
            soc_xbar.o_MAP(l2_mem.i_INPUT(),"l2_mem",base=ArchytasArch.L2_ADDR_START,size=ArchytasArch.L2_SIZE,rm_base=True)
